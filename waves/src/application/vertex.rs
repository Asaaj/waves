...
impl Vertex {
	...
	pub fn from_vecs_2d(position: nglm::Vec2, color: nglm::Vec4) -> Self {
		let position = nglm::vec3(position.x, position.y, 0.0);
		Self { position, normal: nglm::vec3(0.0, 0.0, 1.0), color }
	}
...
mod tests {
	...
	fn non_origin_mesh() {
		...
		waves_log!("Center: {}", mesh.center.unwrap());
